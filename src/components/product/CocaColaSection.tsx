...
const CocaColaSection = () => {
  ...
  const selectedCardBg = isDark ? 'bg-[#E51D2A]/20' : 'bg-red-50';
  const selectedCardBorder = isDark ? 'border-[#E51D2A]' : 'border-red-500';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-gray-100 to-gray-200 relative overflow-hidden pt-24">
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className={`${modalBg} rounded-2xl max-w-md w-full p-6 md:p-8 shadow-2xl border-2 ${isDark ? 'border-white/20' : 'border-gray-300'} relative animate-scale-in`}>
              ...
              {paymentCards.map((card) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {selectedCard === card.id && <Check className="w-5 h-5 text-[#E51D2A]" />}
                  </div>
                </div>
              ))}
            ...
            <button 
              ...
              className="w-full bg-[#E51D2A] hover:bg-[#c4161f] disabled:bg-[#E51D2A]/50 disabled:cursor-not-allowed text-white py-4 rounded-xl font-bold text-lg transition-all hover:scale-105 active:scale-95 shadow-xl"
            >
              Complete Purchase
            </button>
          </div>
          ...
          <div className={`${modalBg} rounded-2xl max-w-md w-full p-6 md:p-8 shadow-2xl border-2 ${isDark ? 'border-white/20' : 'border-gray-300'} relative animate-scale-in max-h-[90vh] overflow-y-auto`}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className={`w-full ${inputBg} border-2 ${inputBorder} rounded-lg px-4 py-3 ${inputText} ${inputPlaceholder} focus:border-[#E51D2A] focus:outline-none transition-colors`}
                />
              </div>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button 
                ...
                className="flex-1 bg-[#E51D2A] hover:bg-[#c4161f] text-white py-3 rounded-xl font-semibold transition-all hover:scale-105 active:scale-95"
              >
                Confirm Order
              </button>
            </div>
      ...
      {showSuccess && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className={`${modalBg} rounded-2xl max-w-md w-full p-8 shadow-2xl border-2 border-[#E51D2A] text-center animate-scale-in`}>
            <div className="w-20 h-20 bg-[#E51D2A] rounded-full flex items-center justify-center mx-auto mb-6">
              <Check className="w-10 h-10 text-white" />
            </div>
            <h2 className={`text-3xl font-bold ${textPrimary} mb-3`}>Purchase Successful!</h2>
            <p className={`${textMuted} text-lg`}>Your Coca-Cola order has been confirmed</p>
          </div>
        </div>
      )}
