...
const LoadingSpinner: FC = () => {
  const { theme } = useTheme(); // INYECCIÓN DEL TEMA

  const bgColorClass = theme === 'dark'
    ? 'from-gray-900 via-gray-800 to-gray-900'
    : 'from-purple-50 via-white to-blue-50';
  
  const spinnerBorderColor = theme === 'dark' ? 'border-gray-700' : 'border-purple-200';
  
  return (
    <div className={`fixed inset-0 bg-gradient-to-br ${bgColorClass} flex items-center justify-center z-50`}>
      <div className="relative w-32 h-32">
        {/* Static background circle */}
        <div className="absolute inset-0 flex items-center justify-center">
          {/* APLICACIÓN DE CLASE DINÁMICA */}
          <div className={`w-32 h-32 border-8 ${spinnerBorderColor} rounded-full`} />
        </div>
...
const StarRating: FC = () => {
  ...
  return (
    <div className="flex items-center gap-3 pt-4">
      ...
      {/* APLICACIÓN DE CLASE DINÁMICA */}
      <span className={`${textColor} font-semibold text-base lg:text-lg`}>{RATING_VALUE}</span>
      <span className={`${secondaryTextColor} text-sm lg:text-base`}>({REVIEW_COUNT})</span>
    </div>
  );
};
...
const OrganizationLogo: FC<OrganizationLogoProps> = ({ organization, index, isVisible }) => {
  ...
  return (
    <div 
      className="flex items-center gap-2 lg:gap-3 text-white transition-all duration-500" // El texto es blanco, ya que el banner es oscuro
      style={{
  ...
  return (
    <div className={`space-y-6 lg:space-y-8 order-2 lg:order-1 transition-all duration-700 ${
      ...
      <h1 className="text-4xl sm:text-5xl lg:text-7xl xl:text-8xl font-bold leading-tight">
        ...
        <br />
        {/* APLICACIÓN DE CLASE DINÁMICA */}
        <span className={titleColor}>Information</span>
      </h1>
      
      {/* APLICACIÓN DE CLASE DINÁMICA */}
      <p className={`${descriptionColor} text-base sm:text-lg lg:text-xl leading-relaxed max-w-2xl`}>
      ...
      {/* APLICACIÓN DE CLASE DINÁMICA */}
      <p className={`text-sm lg:text-base ${descriptionColor}`}>Trusted software reviewed!</p>
    </div>
  );
};
...
const HeroImage: FC<HeroImageProps> = ({ isVisible }) => (
  <div className={`relative order-1 lg:order-2 transition-all duration-700 delay-200 ${
    ...
    {/* Decorative gradient blurs - YA USAN dark: */}
    ...
    <div className="relative">
      ...
      {/* Floating like icon - OK, no necesita tema */}
      ...
      {/* Floating growth chart icon - YA USA dark: */}
